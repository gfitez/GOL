    ...
    Animal breed(Animal other){
       ...
       for(int rowI=0;rowI<height;rowI++){
          for(int cellI=0;cellI<width;cellI++){
                ...
                if(random(1)>0.9999){//random mutation
...
Animal[] runGen(Animal[] generation){
    ...
    Animal[] parents=Arrays.copyOfRange(generation,0,20);//get top performances
    
    
    Animal[] nextGen=new Animal[generation.length];
    
    
    System.arraycopy(generation,0,nextGen,0,5);//keep 5 best parents

    
    int addAt=5;
    for(int i=0;i<1;i++){
        for(Animal parent : parents){
            nextGen[addAt]=parent.breed(getRandom(parents));
